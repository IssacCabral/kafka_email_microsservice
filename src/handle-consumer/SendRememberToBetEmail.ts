...
class SendRememberToBetEmail extends Mailer {
    constructor() {
        super()
    }

    async sendEmail(message: KafkaMessage, subject: string, fileName: string): Promise<void> {
        const {name, email} = JSON.parse(String(message.value!))

        const data = await ejs.renderFile('/app/src/views/' + fileName, { name, email });
...
export default SendRememberToBetEmail